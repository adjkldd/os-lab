...
// row index start at 0, column index at 1
...
int get_cursor() {
    ...
    //      reg 15: which is low byte of the curosr's offset
    // Once the internal registers has been selected, we may read or
    // write a byte on the data register.
    port_byte_out(REG_SCREEN_CTRL, 14);
    int offset = port_byte_in(REG_SCREEN_DATA) << 8;
    port_byte_out(REG_SCREEN_CTRL, 15);
    offset += port_byte_in(REG_SCREEN_DATA);
    // Since the cursor offset report by the VGA hardware is the number of characters,
...
int handle_scrolling(int cursor_offset) {
    ...
    for (i=1; i<MAX_ROWS; i++) {
        memory_copy(VIDEO_ADDRESS + get_screen_offset(0,i),
		    VIDEO_ADDRESS + get_screen_offset(0,i-1),
                    MAX_COLS *2);
    }
